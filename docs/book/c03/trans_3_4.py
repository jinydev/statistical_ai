...
file_path = r"d:\site\jinydev\Statistical\src\book\c03\3_4_the_marketing_plan\index.md"
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

replacements = {
    r"# 3.4 The Marketing Plan": r"""# 3.4 The Marketing Plan
# 3.4 마케팅 계획""",

    r"We now briefly return to the seven questions about the `Advertising` data that we set out to answer at the beginning of this chapter.":
    r"""We now briefly return to the seven questions about the `Advertising` data that we set out to answer at the beginning of this chapter.
이제 잠시 이 장의 서두 부분에서 해답을 찾고자 제시했던 `Advertising` 데이터 관련 7가지 질문들로 되돌아가 보겠습니다.""",

    r"1. _Is there a relationship between sales and advertising budget?_ This question can be answered by fitting a multiple regression model of `sales` onto `TV` , `radio` , and `newspaper` , as in (3.20), and testing the hypothesis $H_0$ : _β_ `TV` = _β_ `radio` = _β_ `newspaper` = 0. In Section 3.2.2, we showed that the $F$-statistic can be used to determine whether or not we should reject this null hypothesis. In this case the $p$-value corresponding to the $F$-statistic in Table 3.6 is very low, indicating clear evidence of a relationship between advertising and sales.":
    r"""1. _Is there a relationship between sales and advertising budget?_
...
표 3.6 자료에 나타난 이 예제의 $F$-통계치와 연관된 $p$-값 척도는 몹시 매우 낮은 수위이며, 이는 곧 전반적인 광고 예산 활동과 판매량 사이에 아주 확연한 관계가 존재한다는 것을 뚜렷이 증명합니다.""",
    ...
    r"""2. _How strong is the relationship?_
...
이 두 RSE 와 $R^2$ 결과 통계 내역은 함께 묶어 표 3.6 에 전시 도표로 표시되어 있습니다.""",
    ...
    r"3. _Which media are associated with sales?_ To answer this question, we can examine the $p$-values associated with each predictor’s $t$-statistic (Section 3.1.2). In the multiple linear regression displayed in Table 3.4, the $p$-values for `TV` and `radio` are low, but the $p$-value for `newspaper` is not. This suggests that only `TV` and `radio` are related to `sales` . In Chapter 6 we explore this question in greater detail.":
    r"""3. _Which media are associated with sales?_
...
반면 `newspaper` 가 띤 신뢰 구간 잣대엔 영(0) 수치가 포함되어 속해 있는데, 이는 이미 주어진 `TV` 및 `radio` 변수 잣대 값들 여건하에서는 정작 이 변수 하나가 딱히 어떠한 별다른 통계적 유의성마저 확보 내지 지니지 못함을 여실히 대변해 나타냅니다.""",
    ...
    r"We saw in Section 3.3.3 that collinearity can result in very wide standard errors. Could collinearity be the reason that the confidence interval associated with `newspaper` is so wide? The VIF scores are 1 _._ 005, 1 _._ 145, and 1 _._ 145 for `TV` , `radio` , and `newspaper` , suggesting no evidence of collinearity.":
    r"""We saw in Section 3.3.3 that collinearity can result in very wide standard errors.
...
현재 각 `TV`, `radio`, 그리고 `newspaper` 항목별 VIF 계산 점수 지표는 순서대로 각각 $1.005$, $1.145$, 그리고 $1.145$ 에 불과하며, 이는 실상 어느 모델 단면에도 공선성의 폐해를 시사하는 증거 따윈 도무지 일절 없다는 사실을 잘 부연해 제안 설명합니다.""",
    ...
    r"In order to assess the association of each medium individually on sales, we can perform three separate simple linear regressions. Results are shown in Tables 3.1 and 3.3. There is evidence of an extremely strong association between `TV` and `sales` and between `radio` and `sales` . There is evidence of a mild association between `newspaper` and `sales` , when the values of `TV` and `radio` are ignored.":
    r"""In order to assess the association of each medium individually on sales, we can perform three separate simple linear regressions.
...
다만 애초 기저에 널린 여타 `TV` 라거나 혹은 `radio` 따위 여타 변수 인자 값들을 통째 아예 무시해 고려 배제했을 시에 한하여, 저기 `newspaper` 단면과 최종 `sales` 부문 사이에 겨우 그나마 얕고 온화 약소한 미미 연관 기류가 일부 다행스레 좀 존재한다는 증거 여부만이 극소수 관찰 도출될 따름입니다.""",
    ...
    r"5. _How accurately can we predict future sales?_ The response can be predicted using (3.21). The accuracy associated with this estimate depends on whether we wish to predict an individual response, Y = $f(X)$ + \epsilon , or the average response, $f(X)$ (Section 3.2.2). If the former, we use a prediction interval, and if the latter, we use a confidence interval. Prediction intervals will always be wider than confidence intervals because they account for the uncertainty associated with \epsilon , the irreducible error.":
    r"""5. _How accurately can we predict future sales?_
...
주지하듯 예측 구간 범위는 축소 축적 불가결한 이른바 피할 줄일 수 없는 줄일 수 없는 오차(irreducible error) 격인 $\epsilon$ 속성 관련 기저 불확실성 소지 여파까지 모조리 다 총체 감안 동반 계산하므로, 예외 없이 늘 신뢰 구간보다 훨씬 더 넓게 팽창 벌어져 나타납니다.""",
    ...
    r"6. _Is the relationship linear?_ In Section 3.3.3, we saw that residual plots can be used in order to identify non-linearity. If the relationships are linear, then the residual plots should display no pattern. In the case of the `Advertising` data, we observe a non-linear effect in Figure 3.5, though this effect could also be observed in a residual plot. In Section 3.3.2, we discussed the inclusion of transformations of the predictors in the linear regression model in order to accommodate non-linear relationships.":
    r"""6. _Is the relationship linear?_
...
In Section 3.3.2, we showed how to include an interaction term""",
    ...
    r"3.5 Comparison of Linear Regression with _K_ -Nearest Neighbors 111": r"",

    r"in the regression model in order to accommodate non-additive relationships. A small $p$-value associated with the interaction term indicates the presence of such relationships. Figure 3.5 suggested that the `Advertising` data may not be additive. Including an interaction term in the model results in a substantial increase in $R^2$ , from around 90 % to almost 97 %.":
    r"""in the regression model in order to accommodate non-additive relationships.
