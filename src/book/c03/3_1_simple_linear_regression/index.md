...
방정식 3.1 구간에서 $\beta_0$ 와 $\beta_1$ 는 선형 모델에서 각각 모형 _절편(intercept)_ 과 모형 _기울기(slope)_ 항을 나타내는 두 개의 구체적인 미지 상수입니다. 함께 모아 $\beta_0$ 와 $\beta_1$ 는 통상 모델 _계수(coefficients)_ 또는 조작 특정 _파라미터(parameters)_ 로 알려져 있습니다. 일단 우리가 확보한 훈련 데이터를 사용하여 이 모델 계수들에 대한 추정치 $\hat{\beta}_0$ 와 $\hat{\beta}_1$ 를 각각 생성하고 나면, 다음과 같은 체계 구문을 연산 작용함으로써 특정 단위의 TV 광고비 수치를 토대로 향후 미래의 달성 판매량을 곧장 예측해 낼 수 있습니다:
...
여기서 변수 $\hat{y}$ 는 특정 전제 조건 $X = x$ 값에 기초한 산출 도달 결괏값 $Y$ 의 예정 예측 수치를 의미합니다. 이곳에서 우리는 알려지지 않은 형태의 미지 파라미터 혹은 대상 계수에 대한 추정치를 별도 표시하거나, 아니면 최종 해당 범주 응답 변수의 구체적 예측값을 가시적으로 나타내기 위해 특수 _햇(hat)_ 모양 기호인 $\hat{ }$ 표시를 추가해 사용합니다.
