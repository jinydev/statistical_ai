...
선형 회귀 설정에서는 $$R^2$$ = $r^2$ 이 성립합니다. 다시 말해 분산 상관 계수의 제곱수와 통계량 $$R^2$$ 는 서로 동일합니다. 하지만 이어질 다음 절에서 우리는 다중 변수를 사용하는 다중 선형 회귀 문제를 다룰 것이며, 응답 결과를 도출하기 위해 여러 가지 예측 변인수들을 다발적으로 투입해 쓸 것입니다. 이 예측 변인들과 결과 응답 간의 다중 상관성 개념이 이런 환경까지 자동으로 연장되지는 않습니다. 왜냐하면 상관(correlation)이란 것이 많은 수의 복수 변수들 사이의 관계보단 특정 단일 변위 상수 한 쌍 간의 관계 지표를 수량화하는 측정법이기 때문입니다. 이런 빈자리의 역할마저 $$R^2$$ 가 능히 채워 수행해 주는 것을 향후 계속 발견하게 될 것입니다.

---

## Sub-Chapters (하위 목차)

현재 3.1.3 모델의 정확도 평가 소속 문서입니다.
