...
일례로 우리는 저 궤도를 통해 `limit` 지표에 해당하는 실제 참 계숫값이 거의 기필코 $0.15$ 와 $0.20$ 사이 어딘가에 자리 잡고 있음을 미루어 볼 수 있습니다.""",
    ...
    r"""In contrast, the right-hand panel of Figure 3.15 displays contour plots of the RSS associated with possible coefficient estimates for the regression of `balance` onto `limit` and `rating`, which we know to be highly collinear.
...
예를 들어, 개별 계수 측면으로만 떼어놓고 본다면 $limit$ 와 $rating$ 의 실질적인 참 계수 잣대가 각기 $-0.1$ 과 $1$ 수위를 보일 거라 충분히 수긍할 수 있긴 하겠지만, 실제로 이 둘이 그처럼 조합된 쌍으로 동시에 나타날 거라 기대하진 않을 것입니다.""",
    ...
    r"Since collinearity reduces the accuracy of the estimates of the regression coefficients, it causes the standard error for \hat{\beta} _j_ to grow. Recall that the $t$-statistic for each predictor is calculated by dividing \hat{\beta} _j_ by its standard":
    r"""Since collinearity reduces the accuracy of the estimates of the regression coefficients, it causes the standard error for $\hat{\beta}_j$ to grow.
...
Recall that the $t$-statistic for each predictor is calculated by dividing $\hat{\beta}_j$ by its standard""",
    ...
    r"error. Consequently, collinearity results in a decline in the $t$-statistic. As a result, in the presence of collinearity, we may fail to reject $H_0$ : $\beta_j$ = 0. This means that the _power_ of the hypothesis test—the probability of correctly power detecting a _non-zero_ coefficient—is reduced by collinearity.":
    r"""error.
...
The VIF for each variable can be computed using the formula""",
    ...
    r"multiInstead": r"Instead",
    r"variance the": r"the",
    r"inflation variance": r"variance",
    r"factor which": r"which",

    r"collinearity variance inflation factor ": r"",

    r"where _RX_[2] _j |X−j_[is][the] _[R]_[2][from][a][regression][of] _[X][j]_[onto][all][of][the][other] predictors. If _RX_[2] _j |X−j_[is][close][to][one,][then][collinearity][is][present,][and][so] the VIF will be large.":
    r"""where $R_{X_j | X_{-j}}^2$ is the $R^2$ from a regression of $X_j$ onto all of the other predictors.
...
만일 이 산입 결과 $R_{X_j | X_{-j}}^2$ 지표값이 $1$ 이란 수극선에 아주 고조 근막 다다를 만치 가깝게 도달한다면 필경 공선성이 강하게 존재(present)한다는 확증이며, 자연히 그에 부수 연동 계산된 VIF 수치도 대거 극히 높게 수직 커질 것입니다.""",
    ...
    r"3.4 The Marketing Plan 109": r"",

    r"In the `Credit` data, a regression of `balance` on `age` , `rating` , and `limit` indicates that the predictors have VIF values of 1.01, 160.67, and 160.59. As we suspected, there is considerable collinearity in the data!":
    r"""In the `Credit` data, a regression of `balance` on `age`, `rating`, and `limit` indicates that the predictors have VIF values of $1.01$, $160.67$, and $160.59$.
