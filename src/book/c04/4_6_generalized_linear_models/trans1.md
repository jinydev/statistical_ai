...
3장에서 우리는 반응 변수 $Y$가 정량적(quantitative)이라고 가정하고, $Y$를 예측하기 위해 최소 제곱 선형 회귀의 사용을 탐구했습니다.

Thus far in this chapter, we have instead assumed that $Y$ is qualitative.
지금까지 이 장에서 우리는 반대로 $Y$가 정성적(qualitative)이라고 가정해왔습니다.
...
우리는 `mnth` (연중 월), `hr` (하루 중 시간, 0에서 23까지), `workingday` (주말이나 휴일이 아니면 1과 같은 지시 변수), `temp` (섭씨로 정규화된 온도) 및 `weathersit` (맑음; 옅은 안개 또는 흐림; 가벼운 비 또는 가벼운 눈; 혹은 강한 비 또는 폭설의 네 가지 가능한 값 중 하나를 취하는 정성적 변수) 공변량들을 사용하여 `bikers` 변수를 예측하는 것을 고려할 것입니다.

In the analyses that follow, we will treat `mnth`, `hr`, and `weathersit` as qualitative variables.
이어지는 분석에서, 우리는 `mnth`, `hr`, 그리고 `weathersit`을 정성적 변수들로 취급할 것입니다.
