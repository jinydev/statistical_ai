...
[< 4.6 Generalized Linear Models](../index.html) | [4.6.2 Poisson Regression On The Bikeshare Data >](../4_6_2_poisson_regression_on_the_bikeshare_data/trans1.html)
...
시작하기 위해, 우리는 선형 회귀를 사용하여 `bikers`를 예측하는 것을 고려합니다. 그 결과는 표 4.10에 표시됩니다.
...
우리는 예를 들어 날씨가 맑음에서 흐림으로 진행될 경우 평균적으로 시간당 12.89명의 자전거 이용자 감소를 초래한다는 것을 알 수 있습니다. 그러나 날씨가 비나 눈으로 더 진행되면, 이는 추가로 시간당 53.60명 더 적은 이용자를 초래합니다.

We see that bike usage is highest in the spring and fall, and lowest during the winter months.
우리는 자전거 이용이 봄과 가을에 가장 높고 겨울철에 가장 낮다는 것을 알 수 있습니다.

Furthermore, bike usage is greatest around rush hour (9 AM and 6 PM), and lowest overnight.
또한, 자전거 이용은 출퇴근 시간(오전 9시 및 오후 6시) 부근에서 가장 많고 철야 시간대에 가장 낮습니다.
...
따라서, 언뜻 보기에 선형 회귀 모델을 `Bikeshare` 데이터 세트에 적합시키는 것은 합리적이고 직관적인 결과를 제공하는 것처럼 보입니다.

But upon more careful inspection, some issues become apparent.
그러나 더 주의 깊게 관찰해 보면, 몇 가지 문제들이 명백해집니다.
...
예를 들어, `Bikeshare` 데이터 세트의 적합된 값 중 9.6%가 음수입니다: 즉, 선형 회귀 모델은 데이터 세트의 시간 중 9.6% 동안 **음수(negative)**의 사용자 수를 예측합니다.
...
이것은 데이터에 대해 유의미한 예측을 수행하는 우리의 능력에 의문을 제기하며, 또한 계수 추정치, 신뢰 구간, 그리고 회귀 모델의 다른 출력들에 대한 정확성에 대한 우려를 제기합니다.
...
또한, `bikers`의 기댓값이 작을 때는 `bikers`의 분산 역시 작아야 한다고 합리적으로 의심할 수 있습니다.
...
예를 들어, 폭설이 내리는 12월의 오전 2시에는 자전거를 사용하는 사람이 극히 적을 것으로 예상하며, 더욱이 그러한 조건에서 사용자 수와 관련된 분산이 거의 없을 것으로 예상합니다.
...
대조적으로, 하늘이 맑은 4월, 5월, 6월의 오전 7시에서 오전 10시 사이에는 평균적으로 243.59명의 사용자가 있으며, 표준편차는 131.7입니다.
...
이것은 $Y = \sum_{j=1}^{p} X_j \beta_j + \epsilon$ 이라는 선형 모델의 여러 가정들에 대한 중대한 위반입니다. 여기서 오차항은 $\epsilon$ 이고 이는 평균이 0이며 공변량들의 함수가 아닌 **일정한(constant)** 분산 $\sigma^2$ 를 가집니다.

Therefore, the heteroscedasticity of the data calls into question the suitability of a linear regression model.
그러므로 데이터의 불균일 분산(heteroscedasticity)은 선형 회귀 모델의 적합성에 의문을 제기합니다.

Finally, the response `bikers` is integer-valued.
마지막으로, 반응 변수 `bikers`는 정수 값입니다.

But under a linear model, $Y = \beta_0 + \sum_{j=1}^{p} X_j \beta_j + \epsilon$, where $\epsilon$ is a continuous-valued error term.
그러나 선형 모델 하에서 $Y = \beta_0 + \sum_{j=1}^{p} X_j \beta_j + \epsilon$이며, 여기서 오차항 $\epsilon$은 연속적인 값의 오차항입니다.

This means that in a linear model, the response $Y$ is necessarily continuous-valued (quantitative).
이것은 선형 모델에서 타깃 반응 $Y$가 필연적으로 연속적인 값(정량적)임을 의미합니다.
...
따라서 반응 변수 `bikers`의 정수적인 성질은 이 데이터 세트에 대해 선형 회귀 모델이 완전히 만족스럽지 않음을 시사합니다.
...
`Bikeshare` 데이터에 선형 회귀 모델을 적합시킬 때 발생하는 일부 문제들은 반응 변수를 변환함으로써 극복할 수 있습니다; 예를 들어, 우리는 다음과 같이 모델을 적합시킬 수 있습니다.
...
반응 변수를 변환하면 음수 예측의 가능성을 피할 수 있으며, 변환되지 않은 데이터의 불균일 분산 문제의 상당 부분을 극복할 수 있습니다.
...
그러나 이것이 아주 만족스러운 해결책은 아닙니다. 왜냐하면 예측과 추론이 반응 변수 자체가 아니라 반응 변수의 로그값의 관점에서 이루어지기 때문입니다.
...
더욱이 반응 변수에 대한 로그 변환은 반응 변수가 0의 값을 가질 수 있는 설정에서는 절대 적용될 수 없습니다.
...
우리는 다음 섹션에서 포아송 회귀 모델이 이 작업을 위해 훨씬 더 자연스럽고 우아한 접근법을 제공한다는 것을 확인하게 될 것입니다.
