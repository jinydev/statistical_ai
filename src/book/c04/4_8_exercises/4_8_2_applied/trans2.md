...
> 💡 **Vibe Coding:** 
> 4장의 배운 내용들을 바탕으로, 이제 실제 데이터를 다뤄볼 시간입니다! 주식 데이터(`Weekly`), 자동차 연비(`Auto`), 보스턴 주택 범죄율(`Boston`) 데이터 등을 통해 로지스틱 회귀부터 KNN까지 파이썬으로 적용해봅시다. 앞의 실습(Lab)을 잘 따라오셨다면 코드 복습용으로 완벽할 거예요.
...
> 📝 **Vibe Check:** 
> 이전 Lab에서 사용했던 `Smarket`이 매일의 주식 데이터였다면, 이번 연습문제의 `Weekly`는 주간(weekly) 단위로 모아놓은 주식 데이터예요. `Smarket` 실습 파트를 완전히 똑같이 따라가면서 데이터를 조금 더 큰 스케일로 돌려보시면 됩니다.
...
> 📝 **Vibe Check:** 
> 머신러닝의 타겟 데이터를 만드는 기초적인 방법! 연속형 수치가 있을 때 보통 이를 '분류' 문제로 변환하기 위해 타겟 데이터의 기준(중앙값)을 잡고 1과 0의 카테고리로 나누게 되는데, 이를 직접 실습해보는 단계입니다.

   - (b) Explore the data graphically in order to investigate the association between `mpg01` and the other features. Which of the other features seem most likely to be useful in predicting `mpg01`? Scatterplots and boxplots may be useful tools to answer this question. Describe your findings. 
   - (b) `mpg01`과 다른 피처 간의 연관성을 조사하기 위해 그래픽으로 데이터를 탐색하라. 다른 피처 중 어느 것이 `mpg01`을 예측하는 데 가장 유용할 가능성이 높아 보이는가? 산점도 및 상자 그림(boxplot)은 이 질문에 답하는 데 유용한 도구일 수 단. 당신의 발견을 설명하라.
...
> 📝 **Vibe Check:** 
> 파이썬 기초 코딩 시간입니다! 이 책의 매력 포인트 중 하나는 데이터 분석 뿐만 아니라, "코드를 함수화(`def`)하여 재사용하기 편하게 만드는 법"까지 알려준다는 점이죠.
