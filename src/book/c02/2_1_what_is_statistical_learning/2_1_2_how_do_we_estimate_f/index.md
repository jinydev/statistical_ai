...
Let $x_{ij}$ represent the value of the _j_ th predictor, or input, for observation _i_, where $i = 1, 2, \dots, n$ and $j = 1, 2, \dots, p$.

$x_{ij}$ 를 관측치 _i_ 에 대한 _j_ 번째 예측 변수나 입력의 값이라고 합시다. 여기서 $i = 1, 2, \dots, n$ 이고 $j = 1, 2, \dots, p$ 입니다.

Correspondingly, let $y_i$ represent the response variable for the _i_ th observation.

대응하여, $y_i$ 를 _i_ 번째 관측치에 대한 반응 변수라고 합시다.
...
다시 말해서, 우리는 임의의 관측값 $(X, Y)$ 에 대하여 $Y \approx \hat{f}(X)$ 가 되는 함수 $\hat{f}$ 를 찾기를 원합니다.
